...
module permutation_controller #(
    ...
    // Fix (fusion): wr_en/wr_addr son registrados, asi que el dato
    // tambien debe registrarse. La version original conectaba
    // complex_real/imag directo a la RAM y, con un stream continuo
    // (Bloque 1), cada escritura capturaba la muestra siguiente
    // (corrimiento de +1).
    reg signed [DATA_WIDTH-1:0] wr_real_q;
    reg signed [DATA_WIDTH-1:0] wr_imag_q;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                //
                // Fix (integración B1->B2): el Bloque 1 emite
                // frame_start junto con la primera muestra válida,
                // así que si complex_valid acompaña a frame_start la
                // muestra 0 se captura aquí mismo. También se sigue
                // aceptando frame_start un ciclo antes del dato.
                // --------------------------------------------
                IDLE: begin
                    br_valid   <= 1'b0;
                    wr_counter <= {ADDR_WIDTH{1'b0}};
                    ...
                    if (frame_start) begin
                        ...
                        if (complex_valid) begin
                            wr_en      <= 1'b1;
                            wr_addr    <= {ADDR_WIDTH{1'b0}};
                            wr_real_q  <= complex_real;
                            wr_imag_q  <= complex_imag;
                            wr_counter <= {{(ADDR_WIDTH-1){1'b0}}, 1'b1};
                        end
                    end
                end
                ...
                //
                // Fix (fusion): primero presenta el dato con
                // br_valid=1 durante un ciclo completo y recien
                // entonces evalua br_ready. La version original
                // hacia br_valid<=1 y br_valid<=0 en el mismo
                // flanco cuando br_ready ya estaba en alto, por lo
                // que br_valid nunca se observaba con un consumidor
                // siempre listo.
                // --------------------------------------------
                OUTPUT_VALID: begin
                    if (!br_valid) begin
