...
module tb_permutation_ready_pause;
    ...
    initial begin
        $display("======================================");
        $display(" TEST HANDSHAKE br_ready PAUSADO ");
        $display("======================================");
        ...
        // Escritura segura: datos estables antes del posedge
        ...
        $display("----- CHECK CON PAUSAS EN br_ready -----");

        while (out_count < DEPTH && cycle_count < 1000) begin

            @(negedge clk);
            cycle_count = cycle_count + 1;

            // Pausa antes de aceptar output[2]
            if (out_count == 2 && pause_done_2 == 0 && br_valid == 1) begin
                $display("PAUSA br_ready antes de output[2]");
                br_ready = 0;
                ...
                repeat (3) begin
                    ...
                    if (br_valid !== 1'b1) begin
                        $display("ERROR: br_valid bajó durante pausa en output[2]");
                        error_count = error_count + 1;
                    end

                    if ((br_real !== expected_real[out_count]) ||
                        (br_imag !== expected_imag[out_count])) begin
                        $display("ERROR: dato cambió durante pausa output[2]: real=%0d imag=%0d",
                                 br_real, br_imag);
                        error_count = error_count + 1;
                    end
                end
            ...
            // Pausa antes de aceptar output[5]
            if (out_count == 5 && pause_done_5 == 0 && br_valid == 1) begin
                $display("PAUSA br_ready antes de output[5]");
                br_ready = 0;
                ...
                repeat (4) begin
                    ...
                    if (br_valid !== 1'b1) begin
                        $display("ERROR: br_valid bajó durante pausa en output[5]");
                        error_count = error_count + 1;
                    end

                    if ((br_real !== expected_real[out_count]) ||
                        (br_imag !== expected_imag[out_count])) begin
                        $display("ERROR: dato cambió durante pausa output[5]: real=%0d imag=%0d",
                                 br_real, br_imag);
                        error_count = error_count + 1;
                    end
                end
        ...
        if (out_count != DEPTH) begin
            $display("RESULTADO FINAL HANDSHAKE: ERROR, timeout out_count=%0d cycle_count=%0d",
                     out_count, cycle_count);
        end else if (error_count == 0) begin
            $display("RESULTADO FINAL HANDSHAKE: PASS");
        end else begin
            $display("RESULTADO FINAL HANDSHAKE: ERROR, fallos=%0d", error_count);
        end
